...
<Code lang="jsson" code={`employees [
...
<Code lang="jsson" code={`// Good ✅ - Simple template, complex logic in map
users [
  template { name, age }
  map (u) = {
    name = u.name
...
<Code lang="jsson" code={`// users.jsson
users [
